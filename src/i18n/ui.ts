...
export const nav: Record<'accueil' | 'menu' | 'carte' | 'decouvrir' | 'aPropos' | 'avis' | 'club' | 'reservation' | 'faq' | 'contact', LocalizedText> = {
  accueil: { fr: 'Accueil', en: 'Home', ar: 'الرئيسية' },
  menu: { fr: 'Menu', en: 'Menu', ar: 'القائمة' },
    // Full printable "à la carte" listing (see CarteContent.astro's
    // metaTitle "Notre carte" / "Our menu") — distinct from the interactive
    // /menu dish-discovery page above; used by Header.astro's top nav.
